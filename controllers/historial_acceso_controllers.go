...
func UpdateHistorial_acceso(w http.ResponseWriter, r *http.Request){
		...
		_, err := config.DB.Exec(
			"UPDATE historial_acceso SET fecha_intento=$1,existoso=$2, ip_origen=$3, fallo_motivo=$4 WHERE id_historial_acceso=$5",
...
func DeleteHistorial_acceso(w http.ResponseWriter, r *http.Request) {
	...
	_, err := config.DB.Exec("DELETE FROM historial_acceso WHERE id_historial_acceso = $1", id)
